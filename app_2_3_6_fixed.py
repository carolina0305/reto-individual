...
def validar(codigo: int):
    ...
    - Si no coincide: devuelve HTML indicando código erróneo.
    """
    if codigo == BIRTH_YEAR:
        # Devolvemos HTML explícito para acceso concedido
        return f"<h1>Acceso concedido a {ASSISTANT_NAME}</h1>", 200, {"Content-Type": "text/html; charset=utf-8"}
    # Respuesta cuando no coincide: HTML según la petición
